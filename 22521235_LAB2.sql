...
DECLARE  CUR_In_DSGV CURSOR FOR SELECT MSGV FROM GIAOVIEN
DECLARE @MSGV CHAR(5)

OPEN CUR_In_DSGV

FETCH NEXT FROM CUR_In_DSGV INTO @MSGV
WHILE @@FETCH_STATUS = 0
BEGIN
	SELECT MSGV, TENGV, DIACHI, SODT, TENHH
	FROM GIAOVIEN G JOIN HOCHAM H ON G.MSHH = H.MSHH
	WHERE MSGV = @MSGV
	FETCH NEXT FROM CUR_In_DSGV INTO @MSGV
END

CLOSE CUR_In_DSGV
DEALLOCATE CUR_In_DSGV

-- 4.	Cho biết danh sách đề tài gồm MSDT, TENDT và số lượng sinh viên thực hiện của mỗi đề tài (nếu có).
DECLARE CUR_In_DSDT_SLSV CURSOR FOR SELECT MSDT, TENDT FROM DETAI
DECLARE @MSDT CHAR(6), @TENDT NVARCHAR(30)

OPEN CUR_In_DSDT_SLSV 

FETCH NEXT FROM CUR_In_DSDT_SLSV INTO @MSDT, @TENDT
WHILE @@FETCH_STATUS = 0 
BEGIN

	SELECT D.MSDT, D.TENDT, COUNT(SD.MSSV) AS SLSV
    FROM DETAI D JOIN SV_DETAI SD ON D.MSDT = SD.MSDT
    WHERE D.MSDT = @MSDT
    GROUP BY D.MSDT, D.TENDT
    HAVING COUNT(SD.MSSV) > 0

	FETCH NEXT FROM CUR_In_DSDT_SLSV INTO @MSDT, @TENDT
END

CLOSE CUR_In_DSDT_SLSV
DEALLOCATE CUR_In_DSDT_SLSV

-- 5.	Cho biết số lượng đề tài (SLDT) đã hướng dẫn ứng với từng giáo viên (TENGV).
DECLARE CUR_Dem_SLDT_GV CURSOR FOR SELECT TENGV FROM GIAOVIEN
DECLARE @TENGV NVARCHAR(30), @SLDT INT = 0

OPEN CUR_Dem_SLDT_GV

FETCH NEXT FROM CUR_Dem_SLDT_GV INTO @TENGV
WHILE @@FETCH_STATUS = 0
BEGIN
    SELECT  @SLDT = COUNT(*)
    FROM GIAOVIEN GV JOIN GV_HDDT HD ON GV.MSGV = HD.MSGV
    WHERE GV.TENGV = @TENGV

    PRINT @TENGV + ': ' + CAST(@SLDT AS NVARCHAR)

    FETCH NEXT FROM CUR_Dem_SLDT_GV INTO @TENGV
END
CLOSE CUR_Dem_SLDT_GV
DEALLOCATE CUR_Dem_SLDT_GV

-- 6.	Liệt kê danh sách giáo viên (MSGV, TENGV) chưa hướng dẫn đề tài nào. 
DECLARE @MSGV NVARCHAR(30), @TENGV NVARCHAR(30)
...
FETCH NEXT FROM CUR_GV INTO @MSGV, @TENGV
WHILE @@FETCH_STATUS = 0
BEGIN
    PRINT @MSGV + ': ' + @TENGV
...
DECLARE CUR_Tinh_DTB CURSOR FOR SELECT MSDT FROM DETAI
DECLARE @MSDT CHAR(6)

OPEN CUR_Tinh_DTB
FETCH NEXT FROM CUR_Tinh_DTB INTO @MSDT
WHILE @@FETCH_STATUS = 0
BEGIN
	DECLARE @DTB NUMERIC(5,2) = 0
	SELECT @DTB = AVG(DIEM)
	FROM 
	(
	SELECT DIEM FROM GV_HDDT WHERE MSDT = @MSDT
